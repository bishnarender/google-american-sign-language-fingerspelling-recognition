...
#         fidx = np.random.randint(len(self.finger_indices), size=self.n_fingers)
        ...
        # fidx.shape => (2, 6)
        # self.finger_indices[fidx].shape => (2, 6, 4)
        drop_indices = self.finger_indices[fidx].flatten()
        # drop_indices[:5] => [78 79 80 81 53]
...
class TimeShift(BasicTransform):
    ...
    def apply(self, data, shift_rate=5, **params):
        ...
        if shift_rate > 0:
            ...
            # zeros.shape, data.shape => torch.Size([3, 130, 3]), torch.Size([106, 130, 3])
            new_x = torch.cat([zeros,data.clone()])
            # new_x.shape => torch.Size([109, 130, 3])            
...
class OnWindows(BasicTransform):
    ...
    def apply(self, data, window_sizes=[0.5], window_offsets_01=[0.25], force_apply= False, **params):        
        # data.shape => torch.Size([162, 130, 3])
        # window_sizes, window_offsets_01 => [0.4205725853935725, 0.2563471891264172], [0.7881850411221675, 0.5913335143292419]
        
        l = data.shape[0]
        x_new = data.clone()     
        
        if 1:#self.transform_p and random.random() < self.p: # 1 my_
            t = self.transform
#             print(window_sizes,window_offsets_01)
            for mask_size, mask_offset_01 in zip(window_sizes, window_offsets_01):
                ...
                max_mask = np.clip(l-mask_size, 1, l)
                mask_offset = int(mask_offset_01 * max_mask)
                # mask_size, max_mask, mask_offset => 68, 94, 73
    ...
    def get_params(self):
        ...
        # num_windows, window_sizes, window_offsets_01 => 2, [0.4205725853935725, 0.2563471891264172] [0.7881850411221675, 0.5913335143292419]
...
class PoseDrop2(BasicTransform):
    ...
    def __init__(
        ...
        # different values of 'l' along the loop =>
        # x_pose_12, x_pose_14, x_pose_16, x_pose_18, x_pose_20, x_pose_22, x_pose_11, x_pose_13, x_pose_15, x_pose_17, x_pose_19, x_pose_21                
        
        # pose => RARMS + RARMS
        pose_indices = np.array([t for t,l in enumerate(landmarks) if 'pose' in l])
        # pose_indices => [118 119 120 121 122 123 124 125 126 127 128 129]
        
        # pose_indices.shape => (12,)                             
        pose_indices = pose_indices.reshape(-1, 2).T
        # pose_indices.shape => (2, 6)                 
        self.pose_indices_type1 = pose_indices[:,0:].T.reshape(-1)
        # self.pose_indices_type1.shape => (12,)
        self.pose_indices_type2 = pose_indices[:,1:].T.reshape(-1)
        # self.pose_indices_type2.shape => (10,)
        self.pose_indices_type3 = pose_indices[:,2:].T.reshape(-1)
        # self.pose_indices_type3.shape => (8,)
        self.pose_indices_type4 = pose_indices[:,3:].T.reshape(-1)
        # self.pose_indices_type4.shape => (6,)
