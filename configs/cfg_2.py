...
cfg.eval_epochs = 10 # 10 # 1 my_
# augs & tta

# Postprocess
cfg.post_process_pipeline =  "pp_2"
cfg.pp_min_conf = 0.16 #0.15 # 0.16 my_
# replace poor predictions with a dummy phrase which has a small levensthein distance to the train/ test data.
# "https://www.kaggle.com/code/anokas/static-greedy-baseline-0-157-lb" showed '2 a-e -aroe' is a good candidate phrase.
cfg.dummy_phrase_ids = [char_to_num[c] for c in '2 a-e -aroe']
cfg.max_len_for_dummy = 16 #15 # 16 my_
cfg.metric = "metric_1"
...
cfg.train_aug = A.Compose([
                       A.Resample(sample_rate=(0.3,2.), p=0.8),
                       A.DynamicResample(sample_rate=(0.9,1.1), windows=(20,30), p=0.25),
                       A.TimeShift(p=0.5), # 0.5 # 1. my_
...
#                        A.TemporalCrop(length=cfg.max_len,p=0.5),
                       ...
                       ## mask finger landmarks i.e., assign 0.0 value to "x,y and z" of finger landmarks.
                       ...
                       A.OneOf([
                           A.OneOf([
                               # mask pose landmarks i.e., assign 0.0 value to "x,y and z" of pose landmarks.
