...
# https://github.com/ChristofHenkel/kaggle-asl-fingerspelling-1st-place-solution/blob/main/models/mdl_1_pt.py
...
class Decoder(nn.Module):
    ...
    def forward(self, x, labels=None, attention_mask = None, encoder_attention_mask = None):
        ...
        # labels[:2]   =>
        # tensor([[62, 54, 46, 46, 35, 50, 18, 20, 62, 12, ..., 62, 59, 59, 59, 59, 59, 62, 62, 59, 59, 59, 59, 59, 59],
        #         [20, 22, 36, 16, 62, 62, 40, 45, 34, 62, ..., 59, 59, 59, 59, 59, 62, 59, 62, 59, 59, 59, 59, 59, 59]],
        #        device='cuda:0')
        
        
        # self.config.decoder_start_token_id, self.config.pad_token_id => 60, 59

        
        if labels is not None:
            # shift_tokens_right => shift input ids one token to the right.
            decoder_input_ids = shift_tokens_right(labels, self.config.pad_token_id, self.config.decoder_start_token_id)
            # decoder_input_ids[:2] => 
            # tensor([[60, 62, 54, 46, 46, 35, 50, 18, 20, 62, ..., 59, 62, 59, 59, 59, 59, 59, 62, 62, 59, 59, 59, 59, 59],
            #         [60, 20, 22, 36, 16, 62, 62, 40, 45, 34, ..., 61, 59, 59, 59, 59, 59, 62, 59, 62, 59, 59, 59, 59, 59]],
            #        device='cuda:0')            
            # decoder_input_ids.shape => torch.Size([64, 33])
...
class RelPositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int = 512, max_len: int = 5000) -> None:
        ...
        self.pe = None
        # d_model => 144 # embedding
        # max_len => 5000
        # torch.tensor(0.0).expand(1, max_len).shape => torch.Size([1, 5000])        
        self.extend_pe(torch.tensor(0.0).expand(1, max_len))

    def extend_pe(self, x):
        if self.pe is not None:
            if self.pe.size(1) >= x.size(1) * 2 - 1:
                # self.pe.dtype, x.dtype, self.pe.device, x.device => torch.float32, torch.float32, cpu, cuda:0
                if self.pe.dtype != x.dtype or self.pe.device != x.device:
                    self.pe = self.pe.to(dtype=x.dtype, device=x.device)
                return

        pe_positive = torch.zeros(x.size(1), self.d_model)
        # pe_positive.shape => torch.Size([5000, 144])
        pe_negative = torch.zeros(x.size(1), self.d_model)
        
        # position represents a tensor containing sequential positions. and div_term is a tensor used to scale the positions.
        position = torch.arange(0, x.size(1), dtype=torch.float32).unsqueeze(1)
        # position.shape => torch.Size([5000, 1])
        # position[:6] => tensor([[0.], [1.], [2.], [3.], [4.], [5.]])
        
        # div_term => values based on the exponential decay of the constant term 10000.0.
        #          => values are used in the subsequent calculations of the positional encodings to ensure that they vary smoothly across different positions.
        div_term = torch.exp(
            torch.arange(0, self.d_model, 2, dtype=torch.float32) * -(math.log(10000.0) / self.d_model)
        )
        
        # div_term.shape => torch.Size([72])
        # div_term[:5] => tensor([1.0000, 0.8799, 0.7743, 0.6813, 0.5995])
        
        # (position * div_term).shape => torch.Size([5000, 72])            
        
        # even-indexed dimensions (0-based indexing) of pe_positive are filled with sine values, 
        pe_positive[:, 0::2] = torch.sin(position * div_term)
        # odd-indexed dimensions (0-based indexing) of pe_positive are filled with cosine values, 
        pe_positive[:, 1::2] = torch.cos(position * div_term)
        
        # pe_positive.shape => torch.Size([5000, 144])
        
        # sine and cosine values for the negative positions.
        pe_negative[:, 0::2] = torch.sin(-1 * position * div_term)
        pe_negative[:, 1::2] = torch.cos(-1 * position * div_term)
                
        # [0] => axis to flip on i.e., reverse the values of columns.
        pe_positive = torch.flip(pe_positive, [0]).unsqueeze(0)
        
        # pe_positive.shape => torch.Size([1, 5000, 144])
        
        # [1:] => leaving first row as the first row of "pe_negative" and last row of "torch.flip(pe_positive, [0])" is same.
        pe_negative = pe_negative[1:].unsqueeze(0)
        # pe_negative.shape => torch.Size([1, 4999, 144])
        pe = torch.cat([pe_positive, pe_negative], dim=1)
        # pe.shape => torch.Size([1, 9999, 144])
        # pe[0, :, 0] => tensor([-0.6639,  0.2705,  0.9563, 0.7629, -0.1320  ..., 0.1320, -0.7629, -0.9563, -0.2705,  0.6639])
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        # x.shape => torch.Size([64, 384, 144])
        
        self.extend_pe(x)        
        # 9999 // 2 = 4999; 4999-384+1 = 4616; 4999+384 = 5383; 5383-4616 = 767
        
        # positions around the central position within the sequence. central position is typically chosen as a reference point because it represents the position of the token being considered in relation to other tokens. Relative positional encodings help the model understand how tokens interact with their neighbors.
        pos_emb = self.pe[:, self.pe.size(1) // 2 - x.size(1) + 1 : self.pe.size(1) // 2 + x.size(1)]
        
        # pos_emb.shape => torch.Size([1, 767, 144])
        return pos_emb
...
class RelativeMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        # self.num_heads, self.d_head => 4, 36
        self.u_bias = nn.Parameter(torch.Tensor(self.num_heads, self.d_head))
        # self.u_bias.shape => torch.Size([4, 36])        
    ...
    def forward(
        ...
        # pos_embedding.shape => torch.Size([64, 767, 144])
        # self.pos_proj(pos_embedding).shape => torch.Size([64, 767, 144])
        pos_embedding = self.pos_proj(pos_embedding).view(batch_size, -1, self.num_heads, self.d_head)
        # pos_embedding.shape => torch.Size([64, 767, 4, 36])
        # pos_embedding.permute(0, 2, 3, 1).shape => torch.Size([64, 4, 36, 767])
        

        content_score = torch.matmul((query + self.u_bias).transpose(1, 2), key.transpose(2, 3))
        pos_score = torch.matmul((query + self.v_bias).transpose(1, 2), pos_embedding.permute(0, 2, 3, 1))
        # pos_score.shape => torch.Size([64, 4, 384, 767])
        pos_score = self._relative_shift(pos_score)

        score = (content_score + pos_score) / self.sqrt_dim
        
        # score.shape => torch.Size([64, 4, 384, 384])
        # mask.shape => torch.Size([64, 384, 384])
        
        if mask is not None:
            mask = mask.unsqueeze(1)
            # mask.shape => torch.Size([64, 1, 384, 384])
            score.masked_fill_(mask, -1e9) # -1,000,000,000        
...
class MultiHeadedSelfAttentionModule(nn.Module):
    ...
    def forward(self, inputs: Tensor, mask: Optional[Tensor] = None):
        ...
        # pos_embedding.shape => torch.Size([1, 767, 144])
...
class SqueezeformerBlock(nn.Module):
    ...
    def forward(self, x, mask):
        # mask.shape => torch.Size([64, 384])
        mask_pad = ( mask).long().bool().unsqueeze(1)        
        mask_pad = ~( mask_pad.permute(0,2,1) * mask_pad)     
        
        mask_flat = mask.view(-1).bool()
        # mask_flat.shape => torch.Size([24576]) # BS=64 # 64*384 = 24576 
        bs, slen, nfeats = x.shape
        # bs, slen, nfeats => 64, 384, 144
        ...
        # x_skip.shape => torch.Size([24576, 144])
        x = x_skip[mask_flat].unsqueeze(0)        
        # x.shape => torch.Size([1, 9934, 144])
        x = self.ln_mhsa(x)    
        # x.shape => torch.Size([1, 9934, 144])
...
class FeatureExtractor(nn.Module):
    def __init__(self, n_landmarks, out_dim):
        super().__init__()   
    ...
    def forward(self, data, mask):
        ...
        # m.shape => torch.Size([64, 384])
        x = self.stem_linear(xc)

        
        # batchnorm without pads
        bs, slen, nfeat = x.shape
        # bs, slen, nfeat => 64, 384, 144
        x = x.view(-1, nfeat)
        # x[mask.view(-1)==1].shape => torch.Size([9934, 144])
        x_bn = x[mask.view(-1)==1].unsqueeze(0)
        # x_bn.shape => torch.Size([1, 9934, 144]) # 9934 => Sequence length.
        x_bn = self.stem_bn(x_bn.permute(0,2,1)).permute(0,2,1)
        # x_bn.shape => torch.Size([1, 9934, 144])
...
class Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        # x.shape => torch.Size([64, 384, 130, 3])
        
        # batch['token_ids'].shape => torch.Size([64, 33])
        # batch['token_ids'][:2] => 
        # tensor([[32, 43, 36, 55, 50, 46, 45, 37, 45, 14, 56, 46, 39, ..., 59, 59, 59, 59, 59, 59, 59, 59],
        #         [32, 50, 39, 32, 45, 51, 40,  0, 44, 36, 45, 35, 46, ..., 59, 59, 59, 59, 59, 59, 59, 59]], ...)
        
        labels = batch['token_ids']    

        # batch['input_mask'].shape => torch.Size([64, 384])
        
        mask = batch['input_mask'].long()
        label_mask = batch['attention_mask']
        
        x[torch.isinf(x)] = 0. # my_
        ...
        # self.decoder_mask_aug => 0.2
        
        if self.training:
            '''
            randomly replace 20% of labels with out-of-frame label.
            62 => random token which is out-of-vocabulary i.e., not present in vocab tokens.
            '''
            m = torch.rand(labels.shape) < self.decoder_mask_aug
            decoder_labels[m] = 62

        logits = self.decoder(x, labels=decoder_labels, encoder_attention_mask=mask) # mask.long()
        ...
        if not self.training:
            ...
            if self.val_mode == 'padded':
                generated_ids = self.decoder.generate(x, max_new_tokens=self.max_phrase + 1, encoder_attention_mask=mask) # mask.long()
